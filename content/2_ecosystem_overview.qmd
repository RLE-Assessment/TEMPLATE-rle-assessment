...
# Terrestrial Ecosystems of Columbia

The Columbia ecosystem typology includes of 64??? ecosystem types across 10??? biomes (Table ???).
...
regions. The Columbia ecosystem typology has a hierarchical structure, where each ecosystem type
...
distribution of Columbia’s ecosystem types is shown in Figure 2.4, and map data is made publicly
...
```{python}
gee_redlist.initialize_ee(project=config['gcp_project'])
```
